...
$AlatasPackageName = 'Z-Alatas-NoDefender-Package'

$AlatasModules = "$env:windir\AlatasModules"
$onlineSxS = "$AlatasModules\Scripts\online-sxs.cmd"
$packagesPath = "$AlatasModules\Packages"
$ProgressPreference = 'SilentlyContinue'
$winlogon = "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon"
$taskName = 'RemoveDefenderFailedAlatas'
...
$packages = (Get-WindowsPackage -online | Where-Object { $_.PackageName -like "*$AlatasPackageName*" }).PackageName
if (!($?)) {
...
function SetSafeMode {
	...
	Set-ItemProperty -Path $winlogon -Name Shell -Value "$env:windir\AlatasDesktop\7. Security\Defender\Toggle Defender.cmd -SafeMode"
	Restart-Computer
	exit
}
...
function InstallPackage {
	$latestCabPath = (Get-ChildItem -Path $packagesPath -Filter "*$AlatasPackageName*.cab" | Sort-Object | Select-Object -Last 1).FullName
	...
	if ($LASTEXITCODE -ne 0) {
		...
		if ($Silent) {
			...
			$action = New-ScheduledTaskAction -Execute "$env:windir\System32\cmd.exe" -Argument '/c "%windir%\AlatasDesktop\7. Security\Defender\Toggle Defender.cmd" -DisableFailedMessage'
...
if ($DisableFailedMessage) {
	...
	$WindowTitle = 'Failed disabling Defender - Alatas'
